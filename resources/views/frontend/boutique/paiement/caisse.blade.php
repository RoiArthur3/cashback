        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="checkout__order">
                    ...
                    // Cashback total (cagnotte globale)
                    $cashback_gross = method_exists($boutique,'computeCashback') ? $boutique->computeCashback($total) : 0;

                    // Part du client (pré-calcul via le même split que la finalisation)
                    $cashback_client = 0;
                    if ($cashback_gross > 0) {
                        // on crée une "commande fantôme" juste pour appeler le split
                        $cmd = new Commande([
                            'cashback_fcfa' => $cashback_gross,
                            'user_id'       => auth()->id(),
                            'boutique_id'   => $boutique->id,
                        ]);
                        // on renseigne les relations pour que le split sache s'il y a commercial/parrain
                        $cmd->setRelation('user', auth()->user());
                        $cmd->setRelation('boutique', $boutique);

                        $splits = \App\Services\CashbackService::split($cmd);
                        // clé 'client' (ou 'buyer' si tu as gardé ce nom)
                        $cashback_client = (int)($splits['client'] ?? ($splits['buyer'] ?? 0));
                    }
                    @endphp
                    ...
                    {{-- ⬇️ AJOUTER ICI : Utilisation du cashback --}}
                    @php
                        ...
                        $maxRedeemByPercent = (int) floor($total * $redeemMaxPercent);
                        $maxRedeem = min($walletBalance, $maxRedeemByPercent);
                    @endphp

                    @if($maxRedeem > 0)
                    <div class="mb-3">
                        <label class="form-label">Utiliser mon cashback</label>
                        ...
                        <input type="number"
                                ...
                                max="{{ $maxRedeem }}"
                                step="1"
                                value="{{ old('use_cashback', 0) }}">
                        <span class="input-group-text">FCFA</span>
                        </div>
                        <small class="text-muted d-block mt-1">
                        Solde disponible : <strong>{{ number_format($walletBalance,0,',',' ') }} FCFA</strong> —
                        Utilisable max : <strong id="maxRedeemDisplay">{{ number_format($maxRedeem,0,',',' ') }} FCFA</strong>
                        ({{ (int)($redeemMaxPercent*100) }}% du total).
                        </small>
                        <div class="mt-2">
                        <span class="text-muted">Total à payer après utilisation :</span>
                        <strong id="totalAfterRedeem">{{ number_format($total,0,',',' ') }} FCFA</strong>
                        </div>
                    </div>
                    @else
                    <div class="alert alert-light border">
                        Votre solde de cashback (<strong>{{ number_format($walletBalance,0,',',' ') }} FCFA</strong>) ne permet
                        pas d’être utilisé sur ce panier (limité à {{ (int)($redeemMaxPercent*100) }}% du total).
                    </div>
                    @endif
                    {{-- ⬆️ FIN AJOUT --}}


                    @if($cashback_client > 0)
                    <div class="alert alert-success py-2">
                        Vous gagnerez <strong>{{ number_format($cashback_client,0,',',' ') }} FCFA</strong> en cashback après paiement.
                    </div>
                    @endif
